...
export default {
    ...
    watch: {
        content: function(newValue, oldVal) {
            this.$emit('transferUser ',newValue)
        }
    },
    mounted() {
    ...
    methods: {
        ...
        setDataVal(){
          ...
          this.$emit('transferUser ',this.content)
        },
        uploadImgs(e) {
            //上传图片
        ...
        upFile(file, type) {
            put(parseInt(Math.random() * 10000) + file.name.substring(0, 20), file).then(result => {
              this.spinning = false

     ...
     <a-table :columns="columns" :data-source="data" :scroll="{ x: 1500, y: 500 }"
        ...
        :loading="loading">
         <div slot="action" slot-scope="text,row" class="actiosn main_color">
           <span @click="setPro(row)">去处理</span>
         </div>
  ...
  export default{
    ...
    methods:{
      ...
      setPro(item){            //去处理
        ...
        if(item.type==1){
          this.$router.push({path:'/ManagementPanel/'+typeObj[item.type],query:{week_id:item.order_id,type:"add"}})
        }else{
          this.$router.push({path:'/ManagementPanel/'+typeObj[item.type],query:{id:item.id}})
        }
      }
    }
  }

  ...
  export default{
    data(){
      return{
        ...
        columns:[
          { title: '学校', width: 200, dataIndex: 'full_name', key: 'full_name', fixed: 'left',align:"center" },
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InfoBookList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写
        ...
        var objs = {
          ...
          pupil_count:item.pupil_count,
          junior_count:item.junior_count
        }
        localStorage.setItem("localInfo",JSON.stringify(objs))
        this.$router.push({path:"/ManagementPanel/t_BookSubscription"})
      },
      lookDetails(item){          //查看
        var years = item.year+"年"+item.semester
        var objs = {
        ...
        localStorage.setItem("localInfo",JSON.stringify(objs))
        this.$router.push({path:'/ManagementPanel/t_BookSubDetails'})
      }
    }
  }
</script>

  ...
  export default{
    data(){
      return{
        ...
        columns:[
          { title: '学校', width: 370, dataIndex: 'full_name', key: 'full_name', fixed: 'left',align:"center" },
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InfoPlaceList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写
        ...
        this.$router.push({path:"/ManagementPanel/t_SiteManageAdd"})
      },
      lookDetails(item){          //查看
        var years = item.year+"年"+item.semester
        var objs = {
        ...
        localStorage.setItem("localInfo",JSON.stringify(objs))
        this.$router.push({path:'/ManagementPanel/t_SiteManageDetails'})
      }
    }
  }
</script>

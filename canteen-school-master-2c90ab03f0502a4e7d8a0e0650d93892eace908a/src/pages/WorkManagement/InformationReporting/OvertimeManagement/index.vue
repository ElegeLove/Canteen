       ...
       <a-table :columns="columns" :data-source="data" :scroll="{ x: 1500, y: 500 }"
          ...
          :loading="loading">
           <div slot="action" slot-scope="text,row,index" class="actiosn main_color">
              ...
              <span v-else @click="lookDetails(row.calendar_id)">查看</span>
           </div>
        </a-table>
    </div>
  </div>
  ...
  export default{
    data(){
      return{
        ...
        columns:[
          { title: '学期', width: "50%", dataIndex: 'semester', key: 'semester',align:"center",
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.infoSupermarketlist(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(row){         //填写
        ...
        this.$router.push({path:"/ManagementPanel/t_OverTimeAdd",query:objs})
      },
      lookDetails(calendar_id){          //查看
        this.$router.push({path:'/ManagementPanel/t_OverTimeDetails',query:{calendar_id}})
      }
    }
  }
</script>

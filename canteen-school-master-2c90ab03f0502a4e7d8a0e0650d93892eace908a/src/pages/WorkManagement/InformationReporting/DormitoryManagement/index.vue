...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InfoRoomList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写
        var objs = {
          year:item.year+"年"+item.semester,
          school_id:item.id,
          calendar_id:item.calendar_id
        }
        this.$router.push({path:"/ManagementPanel/t_DormiAdd",query:objs})
      },
      lookDetails(item){          //查看
        var objs = {
          year:item.year+"年"+item.semester,
        ...
        this.$router.push({path:'/ManagementPanel/t_DormiDetails',query:objs})
      }
    }
  }
</script>

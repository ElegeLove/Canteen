       ...
       <a-table :columns="columns" :data-source="data" :scroll="{ x: 1500, y: 500 }"
          ...
          :loading="loading">
           <div slot="action" slot-scope="text,row,index" class="actiosn main_color">
              ...
              <span v-else @click="editPage(row)">查看</span>
           </div>
        </a-table>
    </div>
  </div>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InfoSchoolList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写-查看
        ...
        this.$router.push({path:"/ManagementPanel/t_BasicAdd",query:{calendar_id:item.calendar_id,years,id:item.school_id}})
      },
      // lookDetails(item){          //查看
      //   var years = item.year+"年"+item.semester
      //   this.$router.push({path:'/ManagementPanel/t_BasicDetails',query:{calendar_id:item.calendar_id,years,id:item.school_id}})
      // }
    }
  }
</script>

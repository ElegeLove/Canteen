...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InfoLabourList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写
        ...
        var objs = {
          ...
          years,
          labour_total_count:item.labour_total_count,
          school_id:item.id
        }
        localStorage.setItem("localInfo",JSON.stringify(objs))
        this.$router.push({path:"/ManagementPanel/t_LaborAdd"})
      },
      lookDetails(item){          //查看
        var years = item.year+"年"+item.semester
        var objs = {
        ...
        localStorage.setItem("localInfo",JSON.stringify(objs))
        this.$router.push({path:'/ManagementPanel/t_LaborDetails'})
      }
    }
  }
</script>

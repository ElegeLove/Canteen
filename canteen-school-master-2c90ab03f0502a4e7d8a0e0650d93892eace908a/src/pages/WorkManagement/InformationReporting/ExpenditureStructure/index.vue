  ...
  export default{
    data(){
      return{
        ...
        columns:[
          { title: '学校', width: 300, dataIndex: 'school', key: 'school',align:"center",fixed: 'left'},
          { title: '学期', width: "35%", dataIndex: 'year', key: 'year',align:"center",
            customRender: (value, row, index) => {
              return row.year+"年"+row.semester
            }
          },
          { title: '月份', width: "35%", dataIndex: 'month', key: 'month',align:"center",
            customRender: (value, row, index) => {
              return row.month+"月"
            }
          },
    ...
    methods:{
      ...
      getList(){                 //获取列表
        this.loading = true
        this.$http.InformationExpenditureList(this.postData).then((res)=>{
           this.loading = false
          if(res.code==1){
      ...
      editPage(item){         //填写
        var objs = {
          ...
          year:item.year,
          month:item.month,
          out_month:item.out_month,
          out_year:item.out_year,
          this_month_human_total:item.this_month_human_total,
          this_month_other_total:item.this_month_other_total
        }
        localStorage.setItem("localdatas",JSON.stringify(objs));
        this.$router.push({path:"/ManagementPanel/t_ExpendiyureAdd"})
      },
      lookDetails(item){          //查看
        var objs = {
          school:item.school,
        ...
        localStorage.setItem("localdatas",JSON.stringify(objs))
        this.$router.push({path:'/ManagementPanel/t_ExpenditruDetails',query:{id:item.id}})
      }
    }
  }
</script>

      ...
      <div class="lists"
       ...
       >
        <div class="tops">
           ...
           <div class="rigs">
             <p><span class="danshen">{{it.name}}</span><span @click.stop="goShop(it)">查看店铺</span></p>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      goDetails(item){      //详情
        this.$router.push({path:"/ManagementPanel/SupplierDetails",query:{type:1,id:item.id}})
      },
      goShop(item){       //店铺详情
        this.$router.push({path:"/ManagementPanel/ShopDetails",query:{id:item.id,supplier_id:item.supplier_id}})
      },
      getList(){          //获取列表

<template>
  <div>
    <div class="bottom-20">
      <a-button type="primary">周计划</a-button>
      <a-button @click="$router.push('/procurement/orderPro/byPlan/list/day')">日计划</a-button>
    </div>
    <!--  -->
    <div>
      <!-- 搜索 start -->
      ...
      <a-table
        ...
        :loading="loading"
      >
        <template slot="semester" slot-scope="text,record">{{record.year}}年{{record.semester}}</template>
        ...
        <template slot="action" slot-scope="text,record">
          ...
          <!-- <template v-else-if="record.order_status == 2">
            <a-button
              type="link"
              @click="$router.push(`/procurement/orderPro/byPlan/dealInfo/${record.id}`)"
            >查看</a-button>
          </template> -->
          <template v-if="record.order_status == 0 || record.order_status == 3">
            <a-button
              type="link"
              @click="$router.push(`/procurement/orderPro/byPlan/deal?ids=${record.id}`)"
            >下单</a-button>
          </template>
          <template v-else>
            <a-button

<template>
  <div>
    <div class="bottom-20">
      <a-button @click="$router.push('/procurement/orderPro/byPlan/list/week')"
        >周计划</a-button
      >
      <a-button type="primary">日计划</a-button>
    </div>
    <!--  -->
    <div>
      <!-- 搜索 start -->
      ...
      <!-- 搜索 end -->
      <div class="bottom-20">
        <a-button type="info" @click="goDeal()">批量下单</a-button>
        <span>注:请选择同一学校，同一周进行下单</span>
      </div>
      <a-table
        :columns="columns"
        ...
        <template slot="action" slot-scope="text, record">
          <template v-if="record.order_status == 1">
            <a-button
              ...
              @click="
                $router.push(
                  `/procurement/orderPro/byPlan/dealInfo/${record.id}?type=day`
                )
              "
              >查看</a-button
            >
          ...
          <template v-else>
            <a-button
              ...
              @click="
                $router.push(
                  `/procurement/orderPro/byPlan/deal?ids=${record.id}&type=day`
                )
              "
              >下单</a-button
            >
...
export default {
  data() {
    return {
      columns: [
        ...
        { title: "下单时间", dataIndex: "order_time_text" },
        {
          title: "状态",
    ...
    rowSelection() {
      return {
        onChange: (selectedRowKeys, selectedRows) => {
          this.item_datas = selectedRows;
          this.selectedRowKeys= selectedRowKeys
        },
        getCheckboxProps: (record) => ({
          props: {
            disabled:( record.order_status ==1 || record.order_status ==2) , // Column configuration not to be checked
          },
        }),
      };
    },
  ...
  methods: {
    ...
    goDeal() {
      ...
      var bools = this.item_datas.every((item,index)=>{
        return item.school_id==this.item_datas[0].school_id&&item.school_week==this.item_datas[0].school_week
      })
      if(!bools){
        return this.$message.error("请选择同一学校，同一周进行下单");
      }
      this.$router.push(
        `/procurement/orderPro/byPlan/deal?ids=${this.selectedRowKeys.join(
          ","
        )}&type=day`
      );
    },
    ...
    search(data) {
      ...
      this.$http
        .procurement_orderPro_byPlan_deal_day_list(this.searchForm)
        .then((res) => {
          this.loading = false;
    ...
    cancelOrder(id) {
      this.$confirm({
        ...
        onOk: () => {
          this.$http
            .procurement_orderPro_byPlan_deal_day_cancel({ id: id })
            .then((res) => {
              this.search();
            });
        },

...
export default {
    ...
    methods: {
		regIptsa(item){
			item.real_quantity = this.$commonJs.regMoneyOnes(item.real_quantity,3)
		},
        // 添加食材点击供应商
        ...
        // iptReg(){         //验证
        //   this.addForm.real_quantity = this.$commonJs.regMoney(this.addForm.real_quantity,1)
        // },
        limitNumber(value) {
        ...
        handleOk() {
            ...
            this.tableData.procurementDetail.push({
                budget_id: this.tableData.procurementDetail[0].budget_id,
                budget_week_id: this.tableData.procurementDetail[0]
                    .budget_week_id,
                ...
                supplier_product_id: this.addForm.brand,
                secondary_id: this.addForm.id,
            ...
            this.all_total =
                this.$commonJs.rounds(parseFloat(this.all_total) +
                parseFloat(
                    this.$commonJs.rounds(this.addForm.unit_price * this.addForm.real_quantity,3)
                ),2)
            this.addVisible = false;
        },
        ...
        changeNum(obj, index, num) {
            ...
            this.setAllnum(this.tableData.procurementDetail);
            // } else {
            //     this.tableData.procurementDetail[index].price = 0;
            // obj.real_quantity = 0;
            // }
        },
        getData() {
            this.loading = true;
            this.$http
                .procurement_plan_updateDetail(this.$route.query.type, {
                    id: this.$route.params.id
                })
                .then(res => {
                    this.loading = false;
                    this.tableData = res.data;
                    this.setAllnum(res.data.procurementDetail);
                });
        },
        setAllnum(datas) {
            var sum = 0;
        ...
        submit() {
            ...
            this.$http
                .procurement_plan_updateDetai_save(submitForm.procurementDetail)
                .then(res => {
                    if (res.code == 1) {
                        history.go(-1);
                    }

<template>
  <div class="upBox">
    <a-upload-dragger name="avatar"
    :multiple="multiple"
    :accept="accept"
    listType="picture-card" class="avatar-uploader uplods"
      :showUploadList="false" :customRequest="customRequest" :beforeUpload="beforeUpload" @change="handleChange">
    ...
    <div class="imgBox">
      <div v-for="(item,index) in imageUrl" :key="index" :class="{'imgbo':accept=='image/*','filebo':accept!='image/*'}">
  ...
  export default {
    ...
    methods: {
      ...
      customRequest(action) {
        ...
        if(this.accept!="image/*"){           //图片
          var fileTypes = JSON.parse(JSON.stringify(this.accept)).split(",");
          var upTypes = this.$commonJs.getType(action.file.name,",");
          var indexOfs = fileTypes.indexOf(upTypes);
          if(indexOfs==-1){
            return this.$message.error("只能上传"+this.accept+"格式")
          }
        }
